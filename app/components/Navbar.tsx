...
export default function Navbar() {
  ...
  const coursesSubmenu = [
    { name: "All Degree Programs", href: "/courses", desc: "Undergraduate, Postgraduate & Research Degrees" },
    { name: "MBA Department", href: "/mba-department", desc: "AICTE Approved 2-Year Full-Time Program" },
  ];

  const campusSubmenu = [
    { name: "Facilities & Infrastructure", href: "/facilities", desc: "Library, Computer Labs & Men's Hostel" },
    { name: "Photo & Video Gallery", href: "/gallery", desc: "Campus Events & Silver Jubilee Highlights" },
    { name: "Women Empowerment Cell", href: "/women-empowerment", desc: "Gender Sensitization & Leadership Workshops" },
  ];
        ...
        <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-2">
          <div className="flex items-center gap-4 text-slate-200">
            <span className="flex items-center gap-1.5 hover:text-amber-300 transition-colors">
              ...
              <a href="tel:9976573040" className="font-semibold text-amber-300">
                9976573040
              </a>
            </span>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <div className="relative w-64 sm:w-72 h-20 shrink-0 group-hover:scale-105 transition-transform">
              <Image
                src="/logo.png"
          ...
          <div className="hidden xl:flex items-center gap-1">
                ...
                <div className="absolute top-full left-0 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 p-2 space-y-1 animate-in fade-in zoom-in-95 duration-150 z-50">
                  {coursesSubmenu.map((sub) => (
                    ...
                    >
                      ...
                      <div className="text-[11px] opacity-80 font-normal">{sub.desc}</div>
                    </Link>
                  ))}
                </div>
              )}
            ...
            >
              <button
                className={`flex items-center gap-1 px-3 py-1.5 rounded-md text-sm font-semibold transition-all ${
                  ["/facilities", "/gallery", "/women-empowerment"].includes(pathname)
                    ? "bg-[#EC1C23] text-white shadow-sm"
                    : "text-[#252525] hover:text-[#EC1C23] hover:bg-rose-50/70"
                }`}
              >
        ...
        {/* Mobile Menu Drawer */}
        {isOpen && (
          <div className="xl:hidden bg-white border-b border-slate-200 px-4 pt-3 pb-6 space-y-3 shadow-xl animate-in slide-in-from-top-2 duration-200">
            <div className="space-y-1">
              <Link
                href="/"
                onClick={() => setIsOpen(false)}
                className={`block px-3 py-2 rounded-lg text-sm font-bold ${
                  pathname === "/" ? "bg-[#EC1C23] text-white" : "text-[#252525]"
                }`}
              >
                Home
              </Link>
              <Link
                ...
                onClick={() => setIsOpen(false)}
                className={`block px-3 py-2 rounded-lg text-sm font-bold ${
                  pathname === "/about" ? "bg-[#EC1C23] text-white" : "text-[#252525]"
                }`}
              >
                About
              </Link>
              ...
              <div className="border border-slate-200 rounded-xl p-2 space-y-1 bg-slate-50">
                <span className="text-[11px] font-extrabold text-[#EC1C23] uppercase px-2">Courses & Programs</span>
                {coursesSubmenu.map((sub) => (
                  <Link
                    key={sub.href}
                    href={sub.href}
                    onClick={() => setIsOpen(false)}
                    className={`block px-2.5 py-1.5 rounded-lg text-xs font-semibold ${
                      pathname === sub.href ? "bg-[#2C2B5E] text-white" : "text-[#252525]"
                    }`}
                  >
                    {sub.name}
                  </Link>
              ...
              <div className="border border-slate-200 rounded-xl p-2 space-y-1 bg-slate-50">
                <span className="text-[11px] font-extrabold text-[#EC1C23] uppercase px-2">Campus Life</span>
                {campusSubmenu.map((sub) => (
                  <Link
                    key={sub.href}
                    href={sub.href}
              ...
              <Link
                ...
                onClick={() => setIsOpen(false)}
                className={`block px-3 py-2 rounded-lg text-sm font-bold ${
                  pathname === "/staff" ? "bg-[#EC1C23] text-white" : "text-[#252525]"
                }`}
              >
                Staff
              </Link>
              <Link
                ...
                onClick={() => setIsOpen(false)}
                className={`block px-3 py-2 rounded-lg text-sm font-bold ${
                  pathname === "/admission" ? "bg-[#EC1C23] text-white" : "text-[#252525]"
                }`}
              >
                Admission
              </Link>
              <Link
                ...
                onClick={() => setIsOpen(false)}
                className={`block px-3 py-2 rounded-lg text-sm font-bold ${
                  pathname === "/contact" ? "bg-[#EC1C23] text-white" : "text-[#252525]"
                }`}
              >
                Contact
              </Link>
